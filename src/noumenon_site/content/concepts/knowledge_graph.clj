  ...
  [:section
   [:div.container
    ...
    [:p.lead
     ...
     "Queries traverse between levels naturally — a single query can join "
     "commit history, file structure, and component architecture."]
    [:h2 {:id "three-levels"} "Three levels"]
    [:div.layers-grid {:style "margin-top: 2rem;"}
